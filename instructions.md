...
- To test the mobile apps, log in with the dedicated Ente test account in `~/.config/ente-skills/mobile-test-account.json` (`email`, `password`; older setups keep it in `~/.config/ente-workflow/`) without asking Aman. Read it only at a login screen. Never print, commit or paste the password anywhere else. If the file is missing, ask Aman once. Claude Code doesn't type passwords, so there ask Aman to log in once; the app stays logged in when a new build is installed over it.
